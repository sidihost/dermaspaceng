...
--
-- NOTE: users.id and sessions.id in this database are VARCHAR
-- (legacy), not UUID — every FK below mirrors that.
-- ============================================================

ALTER TABLE sessions
  ADD COLUMN IF NOT EXISTS impersonator_id VARCHAR REFERENCES users(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS impersonator_session_id VARCHAR REFERENCES sessions(id) ON DELETE SET NULL;
