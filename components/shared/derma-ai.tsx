...
function parseActions(content: string): ActionCard[] {
  ...
  if (lower.includes('service') || lower.includes('treatment') || lower.includes('facial')) {
    actions.push({ title: 'Services', description: 'View all', link: '/services', icon: 'sparkles' })
  }
  if (lower.includes('location') || lower.includes('visit') || lower.includes('address')) {
  ...
  if (lower.includes('price') || lower.includes('cost') || lower.includes('package')) {
    actions.push({ title: 'Packages', description: 'See pricing', link: '/packages', icon: 'gift' })
  }
...
function ActionIcon({ type }: { type: string }) {
  const icons: Record<string, JSX.Element> = {
    ...
    gift: <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M21 11.25v8.25a1.5 1.5 0 01-1.5 1.5H5.25a1.5 1.5 0 01-1.5-1.5v-8.25M12 4.875A2.625 2.625 0 1012 10.125M12 4.875A2.625 2.625 0 1012 10.125M12 10.125V21m0-10.875h9.75m-9.75 0H3" /></svg>,
  }
...
export default function DermaAI() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      role: 'assistant',
      content: "Hello! Welcome to Dermaspace. I'm your skincare assistant. How can I help you today?",
      timestamp: new Date(),
      actions: [
        { title: 'Book Now', description: 'Schedule visit', link: '/booking', icon: 'calendar' },
        { title: 'Services', description: 'View all', link: '/services', icon: 'sparkles' },
      ]
    }
  ])
  ...
  const startNewChat = () => {
    // Save current chat if it has messages beyond the initial one
    ...
    setMessages([{
      id: Date.now().toString(),
      role: 'assistant',
      content: "Hello! How can I help you with your skincare needs today?",
      timestamp: new Date(),
      actions: [
        { title: 'Book Now', description: 'Schedule visit', link: '/booking', icon: 'calendar' },
        { title: 'Services', description: 'View all', link: '/services', icon: 'sparkles' },
    ...
    <>
      ...
      >
        <div className="relative group">
          <div className="w-14 h-14 rounded-full bg-gradient-to-br from-[#7B2D8E] to-[#5A1D6A] flex items-center justify-center transition-transform group-hover:scale-105">
      ...
      {isOpen && (
        <div 
          className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[58] md:bg-transparent md:backdrop-blur-none"
      ...
      >
        <div className="w-full h-full bg-white md:rounded-2xl flex overflow-hidden md:border md:border-gray-200">
          ...
          <div className={`absolute md:relative inset-y-0 left-0 w-64 bg-gray-50 border-r border-gray-200 flex flex-col transition-transform duration-300 z-10 ${
          ...
          <div className="flex-1 flex flex-col min-w-0">
            ...
            <div className="flex-shrink-0 bg-[#7B2D8E] px-4 py-3">
              <div className="flex items-center justify-between">
                    ...
                    {isSpeaking && (
                      <div className="flex items-center gap-1">
                        <span className="flex gap-0.5">
                          {[1,2,3].map(i => (
                            <span key={i} className="w-1 h-2 bg-white rounded-full animate-pulse" style={{ animationDelay: `${i * 100}ms` }} />
                          ))}
                        </span>
                ...
                <div className="flex items-center gap-1">
                  ...
                  <button
                    onClick={() => { setIsOpen(false); setShowSidebar(false); }}
                    className="w-9 h-9 rounded-lg flex items-center justify-center bg-white/15 text-white hover:bg-white/25 transition-colors"
                  >
                    <X className="w-5 h-5" />
                  </button>
                </div>
              ...
              <div className="space-y-4">
                        ...
                        <div className="mt-2 flex flex-wrap gap-2">
                          {message.actions.map((action, idx) => (
                            <Link
                              ...
                              className="flex items-center gap-2 px-3 py-2 bg-white border border-gray-200 rounded-xl hover:border-[#7B2D8E] transition-all group"
                            >
                              ...
                              <ArrowRight className="w-3 h-3 text-gray-300 group-hover:text-[#7B2D8E]" />
                            </Link>
                          ))}
                        </div>
                      )}
                ...
                {streamingContent && (
                  <div className="flex justify-start">
                    ...
                    <div className="max-w-[80%]">
                      <div className="bg-white text-gray-700 rounded-2xl rounded-tl-sm border border-gray-100 px-4 py-3">
                        <div 
            ...
            {/* Input */}
            <div className="flex-shrink-0 p-4 pb-6 md:pb-4 bg-white border-t border-gray-100">
              <form onSubmit={handleSubmit} className="flex items-center gap-2">
                ...
                <div className="flex-1 relative">
                  <input
                    ...
                    className="w-full px-4 py-2.5 bg-gray-100 rounded-full text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-[#7B2D8E]/20"
                ...
                <button
                  ...
                  className="w-10 h-10 rounded-full bg-[#7B2D8E] text-white flex items-center justify-center hover:bg-[#6B2278] disabled:opacity-40 disabled:cursor-not-allowed transition-colors flex-shrink-0"
                >
