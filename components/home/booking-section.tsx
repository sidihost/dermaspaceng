      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
          ...
          <div className={`relative flex justify-center transition-all duration-700 ${animateIn ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10'}`}>
            {/* Glow effect */}
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-[#7B2D8E]/5 rounded-full blur-3xl" />
            
            {/* Desktop Browser Mockup */}
            <div className="relative w-full max-w-[480px]">
              <div className="bg-[#1a1a1a] rounded-xl p-1.5 shadow-2xl shadow-black/20">
                {/* Browser Chrome */}
                <div className="bg-[#2a2a2a] rounded-t-lg px-3 py-2 flex items-center gap-2">
                  <div className="flex gap-1.5">
                    <div className="w-2.5 h-2.5 rounded-full bg-[#7B2D8E]/60" />
                    <div className="w-2.5 h-2.5 rounded-full bg-[#7B2D8E]/40" />
                    <div className="w-2.5 h-2.5 rounded-full bg-[#7B2D8E]/20" />
                  </div>
                  <div className="flex-1 mx-2">
                    <div className="bg-[#1a1a1a] rounded-md px-3 py-1 text-[10px] text-gray-500 flex items-center gap-1.5">
                ...
                {/* Dashboard Content */}
                <div className="bg-[#fafafa] rounded-b-lg overflow-hidden">
                  {/* Dashboard Header */}
                  <div className="bg-white border-b border-gray-100 px-4 py-3 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="w-6 h-6 rounded-full bg-[#7B2D8E]">
                        <span className="text-[8px] text-white flex items-center justify-center h-full font-medium">AO</span>
                      </div>
                    </div>
                  </div>
                      ...
                      <div className="w-32 hidden sm:block">
                        <div className="space-y-1">
                          {[
                            ...
                            <div key={i} className={`flex items-center gap-2 px-2.5 py-2 rounded-lg cursor-pointer transition-colors ${item.active ? 'bg-[#7B2D8E] text-white' : 'text-gray-500 hover:bg-gray-100'}`}>
                              <svg className={`w-3.5 h-3.5 ${item.active ? 'text-white' : 'text-gray-400'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                                <path strokeLinecap="round" strokeLinejoin="round" d={item.icon} />
                              </svg>
                              <span className={`text-[10px] font-medium ${item.active ? 'text-white' : 'text-gray-600'}`}>{item.label}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                      ...
                      <div className="flex-1 space-y-3">
                        ...
                        <div className="bg-white rounded-xl p-3 border border-gray-100">
                          <div className="flex items-center justify-between mb-3">
                            <span className="text-xs font-semibold text-gray-900">April 2026</span>
                            <div className="flex gap-1">
                              <button className="w-5 h-5 rounded bg-[#7B2D8E]/10 flex items-center justify-center hover:bg-[#7B2D8E]/20 transition-colors">
                          ...
                          <div className="grid grid-cols-7 gap-1 text-center mb-1.5">
                            {['Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa', 'Su'].map((d, i) => (
                              <div key={i} className="text-[8px] font-medium text-gray-400 py-0.5">{d}</div>
                            ))}
                          </div>
                          {/* Calendar grid */}
                            ...
                            {Array.from({ length: 30 }, (_, i) => {
                              ...
                              return (
                                <button
                                  ...
                                  className={`aspect-square rounded-lg flex items-center justify-center text-[9px] font-medium transition-all relative ${
                                    isSelected 
                                ...
                                >
                                  ...
                                  {hasAppointment && !isSelected && (
                                    <span className="absolute bottom-0.5 w-1 h-1 rounded-full bg-[#7B2D8E]" />
                                  )}
                                </button>
                              )
                            })}
                        ...
                        <div className="bg-white rounded-xl p-3 border border-gray-100">
                          <div className="flex items-center justify-between mb-2">
                            <span className="text-[10px] font-semibold text-gray-900">Available Times</span>
                            <span className="text-[8px] text-[#7B2D8E] font-medium">April {selectedDate}</span>
                          </div>
                          <div className="grid grid-cols-4 gap-1.5">
                            {['09:00', '10:30', '12:00', '14:00', '15:30', '16:00', '17:30', '18:00'].map((time, i) => (
                              <button
                                key={time}
                                className={`px-2 py-1.5 text-[8px] font-medium rounded-lg transition-all ${
                                  i === 2 
                        ...
                        <div className="bg-[#7B2D8E] rounded-xl p-3">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                              ...
                              <div>
                                <p className="text-white text-[10px] font-semibold">Signature Facial</p>
                                <p className="text-white/60 text-[8px]">60 mins</p>
                              </div>
                            </div>
                            <div className="text-right">
                              <p className="text-white text-xs font-bold">N25,000</p>
                            </div>
                          </div>
                          <button className="w-full mt-2.5 py-2 bg-white text-[#7B2D8E] rounded-lg text-[10px] font-semibold hover:bg-white/90 transition-colors flex items-center justify-center gap-1">
